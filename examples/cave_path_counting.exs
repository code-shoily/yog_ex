defmodule CavePathCounting do
  ...
  def run do
    graph =
      ...
      |> Yog.add_node(4, "d")
      |> Yog.add_node(5, "end")
      |> Yog.add_edge_ensure(from: 0, to: 1, with: nil)
      |> Yog.add_edge_ensure(from: 0, to: 2, with: nil)
      |> Yog.add_edge_ensure(from: 1, to: 3, with: nil)
      |> Yog.add_edge_ensure(from: 1, to: 2, with: nil)
      |> Yog.add_edge_ensure(from: 2, to: 4, with: nil)
      |> Yog.add_edge_ensure(from: 1, to: 5, with: nil)
      |> Yog.add_edge_ensure(from: 4, to: 5, with: nil)
