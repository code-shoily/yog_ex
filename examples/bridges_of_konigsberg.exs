defmodule BridgesOfKonigsberg do
  @moduledoc """
  The Seven Bridges of Königsberg problem
  """

  require Yog

  def run do
    # Nodes represent the four land masses (A, B, C, D)
    # Edges represent the seven bridges
    graph =
      Yog.undirected()
      |> Yog.add_node(1, "Island A")
      |> Yog.add_node(2, "Bank B")
      |> Yog.add_node(3, "Bank C")
      |> Yog.add_node(4, "Island D")
      # Bridges
      |> Yog.add_edge_ensure(from: 1, to: 2, with: "b1")
      |> Yog.add_edge_ensure(from: 1, to: 2, with: "b2")
      |> Yog.add_edge_ensure(from: 1, to: 3, with: "b3")
      |> Yog.add_edge_ensure(from: 1, to: 3, with: "b4")
      |> Yog.add_edge_ensure(from: 1, to: 4, with: "b5")
      |> Yog.add_edge_ensure(from: 2, to: 4, with: "b6")
      |> Yog.add_edge_ensure(from: 3, to: 4, with: "b7")

    IO.puts("--- Seven Bridges of Königsberg ---")

    # Check if an Eulerian circuit exists (all even degrees)
    if Yog.Property.Eulerian.has_eulerian_circuit?(graph) do
      IO.puts("Eulerian circuit exists!")
    else
      IO.puts("No Eulerian circuit exists.")
    end

    # Check if an Eulerian path exists (0 or 2 odd degrees)
    if Yog.Property.Eulerian.has_eulerian_path?(graph) do
      IO.puts("Eulerian path exists!")

      case Yog.Property.Eulerian.eulerian_path(graph) do
        {:ok, path} -> IO.puts("Path: #{inspect(path)}")
        {:error, _} -> nil
      end
    else
      IO.puts("No Eulerian path exists either.")
    end

    # Example of a graph that DOES have a circuit
    circuit_graph =
      Yog.undirected()
      |> Yog.add_node(1, "A")
      |> Yog.add_node(2, "B")
      |> Yog.add_node(3, "C")
      |> Yog.add_edge_ensure(from: 1, to: 2, with: nil)
      |> Yog.add_edge_ensure(from: 2, to: 3, with: nil)
      |> Yog.add_edge_ensure(from: 3, to: 1, with: nil)

    IO.puts("\n--- Simple Triangle ---")

    case Yog.Property.Eulerian.eulerian_circuit(circuit_graph) do
      {:ok, circuit} -> IO.puts("Circuit found: #{inspect(circuit)}")
      {:error, _} -> IO.puts("No circuit found")
    end
  end
end

BridgesOfKonigsberg.run()
