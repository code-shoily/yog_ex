...
Benchee.run(
  %{
    "Yog (Label Propagation)" => fn {_, yog} -> Yog.Community.LabelPropagation.detect(yog) end,
    "NetworkX (Label Propagation)" => fn {id, _} ->
      Shared.call(port, "run", %{"graph_id" => id, "algorithm" => "label_propagation"})
    end
  },
  inputs: inputs,
  time: 3,
