...
- **Multigraph Lookup API** — Added query and lookup helpers (`has_node?/2`, `fetch_node/2`, `node_count/1`, `edge_count/1`, `all_edges/1`, `has_edge?/2`, `has_edge_between?/3`) across `Yog.Multi` models.

### Fixed

- **Algorithm & Model Fixes** — Corrected map equation entropy calculation in `Infomap`, random walk step range calculations in `Walktrap` ($t = 1$), auto-insertion of endpoint nodes in `Yog.Multi.Model.add_edge/4`, and direction preservation in `Yog.Functional.Model.embed/2`.
- **Set Operations & Graph Compatibility** — Enforced graph directionality checks (`:directed` vs `:undirected`) across `Yog.Operation` set operations and parameter bounds validation in `power/3`.

### Changed

- **1.0 Module Stabilization Audit** — Completed a comprehensive hardening pass across all 132 modules in YogEx (Core, Traversal, Connectivity, Centrality, Community Detection, Layout & Rendering, Flow, Matching, MST, Graph Properties, IO, and Multigraphs). Enforced strict `%Yog.Graph{}` and `%Yog.DAG{}` type guards, standardized `ArgumentError` exception handling, updated math complexity documentation ($\mathcal{O}(\dots)$), and significantly expanded unit, property, and NetworkX oracle test coverage.
